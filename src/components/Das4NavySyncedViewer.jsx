...
const [speedMs, setSpeedMs] = useState(220);
  ...
  // Left panel: continuous DAS/XAI spectrogram video.
  ...
  const spectrograms = manifestSpecs.length > 0 ? manifestSpecs : sceneSpectrograms;
  ...
  useEffect(() => {
    ...
    map.current = L.map(mapDiv.current, {
      ...
      installDasBaseLayerControl(map.current);
  ...
  useEffect(() => {
    ...
    if (aisCoords.length > 1) {
      L.polyline(aisCoords, {
        ...
        .bindPopup("<b>AIS track completa — JEANOAH</b>")
        .addTo(staticLayer.current);

      aisCoords.forEach((c) => bounds.extend(c));
    }

    // AIS segment used in the DAS/XAI animation
    ...
    if (aisEventCoords.length > 1) {
      L.polyline(aisEventCoords, {
        ...
        .bindPopup("<b>AIS segment used in the DAS/XAI animation</b>")
        .addTo(staticLayer.current);
    ...
    if (dasCoords.length > 1) {
      L.polyline(dasCoords, {
        ...
        .bindPopup("<b>DAS-estimated trajectory — somente trecho válido</b>")
        .addTo(staticLayer.current);
  ...
  useEffect(() => {
    ...
    if (currentFrame.das && validLatLon(currentFrame.das.lat, currentFrame.das.lon)) {
      ...
      dasMarker.current.bindPopup(`
        <b>DAS estimate</b><br/>
        Time: ${currentFrame.time_utc}<br/>
        Channel: ${fmt(currentFrame.das.channel, 0)}<br/>
        Cable: ${currentFrame.das.cable || "-"}<br/>
        Score: ${fmt(currentFrame.das.score)}<br/>
        Error: ${fmt(currentFrame.das.error)}
      `);
    }
  ...
  useEffect(() => {
    ...
    for (const ev of nearby) {
      ...
      L.circleMarker([Number(ev.lat), Number(ev.lon)], {
        ...
        .bindPopup(`
          <b>DAS-XAI flow / perturbation</b><br/>
          Time: ${ev.time_utc}<br/>
          Channel: ${ev.channel}<br/>
          Mapped channel: ${ev.mapped_channel || "-"}<br/>
          Cable: ${ev.mapped_cable || "-"}<br/>
          Score: ${fmt(ev.score)}
        `)
        ...
        <div className="spec-stage">
          {currentSpec?.image_url ? (
            ...
            <div className="spec-empty">
              ...
              <br />
              Run the DAS/XAI pipeline and then build_das4navy_visual_data.py.
            </div>
          )}
          ...
          <div className="spec-info">
            <b>DAS/XAI Spectrogram</b>
            <span>Channel: {fmt(currentSpec?.channel, 0)}</span>
          </div>
        </div>
...
<section className="sync-right">
        <div className="map-title">
          <h1>DAS4NAVY</h1>
          <p>JEANOAH — DAS cables, AIS, trajectory estimate, and channel-time flow</p>
        </div>
        ...
        <div className="control-panel">
          ...
          <div className="legend">
            <p><span className="ln north"></span>North DAS cable</p>
            <p><span className="ln south"></span>South DAS cable</p>
            <p><span className="ln ais"></span>AIS JEANOAH</p>
            <p><span className="ln das"></span>DAS estimate</p>
