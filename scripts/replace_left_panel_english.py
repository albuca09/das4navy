...
backup = jsx.with_suffix(".jsx.bak_replace_left_panel")
backup.write_text(s, encoding="utf-8")

# ---------------------------------------------------------------------
# 1) Replace visible Portuguese text/comments still present
# ---------------------------------------------------------------------
repl = {
...
# Remove any remaining probability element.
s = re.sub(r'\s*<[^>]*>\s*Prob\.\s*:[\s\S]*?</[^>]*>', '', s, flags=re.IGNORECASE)
s = re.sub(r'\s*<[^>]*>\s*Probability\s*:[\s\S]*?</[^>]*>', '', s, flags=re.IGNORECASE)

# ---------------------------------------------------------------------
# 2) New complete left panel
# ---------------------------------------------------------------------
new_left = r'''      <section className="sync-left">
...
left_start = s.find('<section className="sync-left">')
right_start = s.find('<section className="sync-right">')

if left_start < 0:
    raise RuntimeError('Could not find <section className="sync-left">.')

if right_start < 0:
    raise RuntimeError('Could not find <section className="sync-right">.')

s = s[:left_start] + new_left + s[right_start:]
...
print("OK — left panel replaced with English split layout.")
print("Backup:", backup)
